...
`config_dir()/devcontainers/build/<digest[:24]>/`, which `up()` passes as
`--override-config`. Experimentally confirmed: `--override-config` relocates only
`devcontainer.json`, and a referenced `build.dockerfile` still resolves against
the workspace. That is why containment is enforced separately rather than by
snapshotting the tree. Residual execution risk is in-container only, where the
agent already has full control.
