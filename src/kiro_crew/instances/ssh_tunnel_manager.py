...
class SshTunnelManager:
    ...
    async def send_session_bundle(self, instance_id: str, bundle: dict) -> tuple[bool, dict]:
        ...
        **The token never leaves this object.** The request is issued here rather
        ...
        cookie = f"mc_token_{int(local_port)}={token}"
        timeout = aiohttp.ClientTimeout(total=_TRANSFER_TIMEOUT)
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(url, json=bundle, headers={"Cookie": cookie}) as resp:
                    try:
                    ...
                    if resp.status in (401, 403):
                        # Distinguished so the caller can re-mint and retry
            ...
            logger.info(
                ...
                instance_id,
                type(e).__name__,  # never the token, never the bundle
            )
            return False, {
