...
def config_digest(config_path: Path) -> str:
    ...
    AcpClient._maybe_devcontainer_info.
    """
...
def write_build_config(project_dir: str, digest: str) -> Path:
    ...
    * The copy is written from the digest-verified bytes and lives under the
    ...
    ``--override-config`` relocates ONLY devcontainer.json: verified by
    experiment, a referenced ``build.dockerfile`` still resolves against the
    workspace. That is why build inputs are separately required to stay inside
    the hashed tree (assert_build_inputs_contained) — together, every input the
    CLI consumes is either digest-bound or refused.
    """
    cfg = find_devcontainer_config(project_dir)
    if cfg is None:
        raise DevcontainerError(f"no devcontainer config under {project_dir}")
    ...
    out_dir = config_dir() / "devcontainers" / "build" / digest[:24]
